...
class UserServiceImpl @Inject() (userDAO: UserDAO, oAuth2InfoDAO: OAuth2InfoDAO) extends UserService {
    ...
    userDAO.find(profile.loginInfo).flatMap {
      ...
      case None => // Insert a new user
        userDAO.save(User(
          ...
          loginInfo = List(profile.loginInfo),
          firstName = profile.firstName,
          lastName = profile.lastName,
          fullName = profile.fullName,
          email = profile.email,
  ...
  override def linkProviderToUser(userLoginInfo: LoginInfo, providerLoginInfo: LoginInfo) = {
    userDAO.find(userLoginInfo).flatMap {
      case Some(user) => userDAO.save(User(
        user.userID,
        List(providerLoginInfo),
        user.firstName,
        user.lastName,
        user.fullName,
        user.email,
